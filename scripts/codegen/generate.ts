...
// tsconfig.json
/**
 * `providerPatterns` is keyed by every `Provider`, so a new enum member breaks the
 * build until it has a pattern. Existing entries are hand-written host regexes and
 * must survive regeneration, so the region is merged rather than replaced: only
 * providers with no entry gain a placeholder, which matches nothing until it is
 * filled in.
 */
const patternsPath = 'packages/types/ExecutionTypes.ts';
let patterns = read(patternsPath);

const existing = new Map<string, string>();
const region = patterns.match(/\/\/ <auto-generated:patterns>([\s\S]*?)\/\/ <\/auto-generated:patterns>/);

for (const line of region?.[1].split('\n') ?? []) {
	const entry = line.match(/\[Provider\.(\w+)\]:\s*(.+?),?\s*$/);
	if (entry) existing.set(entry[1], entry[2].replace(/,$/, ''));
}

const patternEntries = registry.services
	.map((s) => `\t[Provider.${s.name}]: ${existing.get(s.name) ?? '/^implementation$/i'}`)
	.sort((a, b) => a.localeCompare(b))
	.join(',\n');

patterns = patchRegion(patterns, 'patterns', patternEntries);
